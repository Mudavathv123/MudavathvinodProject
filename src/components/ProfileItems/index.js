import {BsGrid3X3} from 'react-icons/bs'
import {BiCamera} from 'react-icons/bi'
import './index.css'

const ProfileItems = props => {
  const {userDetails, postsList, storiesList, profileOwner} = props
  const {
    userId,
    userName,
    profilePic,
    followersCount,
    followingCount,
    userBio,
    postsCount,
  } = userDetails

  const renderStories = () => (
    <ul className="stories-list-container">
      {storiesList.map(eachStory => (
        <li className="stories-item-container" key={eachStory.id}>
          <img
            src={eachStory.image}
            alt={`${profileOwner} story`}
            className="stories-img"
          />
        </li>
      ))}
    </ul>
  )

  const renderPostList = () => (
    <ul className="user-post-list-container">
      {postsList.length > 0 ? (
        postsList.map(eachPost => (
          <li className="user-post-item" key={eachPost.id}>
            <img
              src={eachPost.image}
              alt={`${profileOwner} post`}
              className="user-post-image"
            />
          </li>
        ))
      ) : (
        <div className="no-post-yet-container">
          <div className="no-post-yet">
            <BiCamera size="26" className="bi-camera" />
          </div>
          <h1 className="no-post-yet-text">No Posts</h1>
        </div>
      )}
    </ul>
  )

  const renderFollwers = () => (
    <div className="profile-container">
      <img
        src={profilePic}
        alt={`${profileOwner} profile`}
        className="min-profile-image"
      />
      <div className="user-profile-container">
        <p className="user-id">{userId}</p>

        <div className="user-profile-image-container">
          <img
            src={profilePic}
            alt={`${profileOwner} profile`}
            className="profile-image"
          />
          <ul className="followers-list-container">
            <li className="followers-item-container">
              {postsCount}
              <span className="posts">posts</span>
            </li>
            <li className="followers-item-container">
              {followersCount}
              <span className="posts">followers</span>
            </li>
            <li className="followers-item-container">
              {followingCount}
              <span className="posts">following</span>
            </li>
          </ul>
        </div>
        <h1 className="username">{userName}</h1>
        <p className="user-bio">{userBio}</p>
      </div>
    </div>
  )

  const renderProfile = () => (
    <>
      <div className="user-profile-top-section">{renderFollwers()}</div>
      {renderStories()}
      <hr />
      <h1 className="post-grid">
        <BsGrid3X3 size="20" />
        Posts
      </h1>
      {renderPostList()}
    </>
  )

  return <div className="profile-items-container">{renderProfile()}</div>
}

export default ProfileItems
