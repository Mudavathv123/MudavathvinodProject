...
import Thamnail from './components/Thamnail'
import './App.css';
...
class App extends Component {
  ...
  state = {
    isTrue:false,
    matchImgUrl:imagesList[0].imageUrl,
    timer: 60,
    score:0,
  ...
  getActiveTabImages() {
    ...
    const filterActiveTabImages = imagesList.filter(eachImg => eachImg.category === isActiveTab)
    return filterActiveTabImages
  }
  ...
  matchImage = id => {
    const {matchImgUrl} = this.state 
    const imgObject = imagesList.filter(eachImg => eachImg.id === id)
    
    const {imageUrl} = imgObject[0]
    if(matchImgUrl === imageUrl){
      const newImg = imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
  ...
  render() {
    const {matchImgUrl,score,timer,isActiveTab,isTrue} = this.state
    const filterActiveTabImages = this.getActiveTabImages()
    return (

      <div className = "image-matches-game-container">
        <Navbar score = {score} timer = {timer}/>
          <div className = "game-thumnail-container">
            {!isTrue && (<>
                <img src = {matchImgUrl} alt = "match" className = "match-image"/>
              <ul className = "tabs-container">
                {
                  tabsList.map(eachTab =>  <Tabs tab = {eachTab} key = {eachTab.tabId} updateActiveTab = {this.updateActiveTab}
                  isActive = {eachTab.tabId === isActiveTab}/> )
                }
              
              </ul>
              <ul className = "thamnail-container">
                {
                  filterActiveTabImages.map(eachImg => <Thamnail images = {eachImg} key = {eachImg.id} matchImage = {this.matchImage}/>)
                }
            ...
            {isTrue &&(<>
              <div className = "trofy-bg-container">
                <img src = "https://assets.ccbp.in/frontend/react-js/match-game-trophy.png" alt = "trophy" className = "trofhy-img" />
                <p className = "your-score">Your Score</p>
                <p className = "score">{score}</p>
                <button className = "playAgain-btn" type = "button" onClick = {this.playAgain}>
