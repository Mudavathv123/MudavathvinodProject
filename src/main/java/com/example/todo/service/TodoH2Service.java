...
public class TodoH2Service implements TodoRepository {
    ...
    public Todo addTodo(Todo todo) {
        ...
        jdbcTemplate.update("insert into todolist(todo,status,priority) values(?,?,?)",
                todo.getTodo(), todo.getStatus(), todo.getPriority());

        return jdbcTemplate.queryForObject("select * from todolist where todo = ? and status = ?", new TodoRowMapper(),
